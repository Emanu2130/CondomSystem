	...
	function Page_Main() {
		...
		if ($qs->Count > 0) {
			...
			if ($Sql <> "") {
				...
				$Value = ew_AdjustSql($qs->getRawUrlDecodedValue("f"));
				if ($Value <> "") {
					$arValue = explode(",", $Value);
					$LnkFldType = $qs->getValue("lft"); // Link field data type
					if (is_numeric($LnkFldType))
						$LnkFldType = intval($LnkFldType);
					$cnt = count($arValue);
					for ($i=0; $i<$cnt; $i++) {
				...
				$Value = ew_AdjustSql($qs->getRawUrlDecodedValue("q"));
				if ($Value <> "")

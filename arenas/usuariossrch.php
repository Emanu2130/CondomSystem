...
// Define page object
$usuarios_search = new cusuarios_search();
$Page =& $usuarios_search;
...
// Create page object
var usuarios_search = new ew_Page("usuarios_search");

// page properties
usuarios_search.PageID = "search"; // page ID
var EW_PAGE_ID = usuarios_search.PageID; // for backward compatibility

// extend page with validate function for search
usuarios_search.ValidateSearch = function(fobj) {
	if (!this.ValidateRequired)
		return true; // ignore validation
	var infix = "";
	elm = fobj.elements["x" + infix + "_user_id"];
	if (elm && !ew_CheckInteger(elm.value))
		return ew_OnError(this, elm, "Entero Incorrecto - User Id");
...
usuarios_search.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
usuarios_search.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
usuarios_search.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
usuarios_search.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="arenas">Buscar Modulo: Usuarios<br><br>
<a href="<?php echo $usuarios->getReturnUrl() ?>">Volver a la lista</a></span></p>
<?php $usuarios_search->ShowMessage() ?>
<form name="fusuariossearch" id="fusuariossearch" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return usuarios_search.ValidateSearch(this);">
<p>
...
<table cellspacing="0" class="ewTable">
	<tr<?php echo $usuarios->user_id->RowAttributes ?>>
		...
		<td<?php echo $usuarios->user_id->CellAttributes() ?>><span class="ewSearchOpr">=<input type="hidden" name="z_user_id" id="z_user_id" value="="></span></td>
		<td<?php echo $usuarios->user_id->CellAttributes() ?>>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="arenas">
<input type="text" name="x_user_id" id="x_user_id" value="<?php echo $usuarios->user_id->EditValue ?>"<?php echo $usuarios->user_id->EditAttributes() ?>>
</span></td>
			</tr></table>
		</td>
	</tr>
	<tr<?php echo $usuarios->usuario->RowAttributes ?>>
		...
		<td<?php echo $usuarios->usuario->CellAttributes() ?>><span class="ewSearchOpr">contiene<input type="hidden" name="z_usuario" id="z_usuario" value="LIKE"></span></td>
...
<input type="text" name="x_usuario" id="x_usuario" size="30" maxlength="25" value="<?php echo $usuarios->usuario->EditValue ?>"<?php echo $usuarios->usuario->EditAttributes() ?>>
	...
	<tr<?php echo $usuarios->clave->RowAttributes ?>>
		...
		<td<?php echo $usuarios->clave->CellAttributes() ?>><span class="ewSearchOpr">contiene<input type="hidden" name="z_clave" id="z_clave" value="LIKE"></span></td>
...
<input type="password" name="x_clave" id="x_clave" size="30" maxlength="25"<?php echo $usuarios->clave->EditAttributes() ?>>
	...
	<tr<?php echo $usuarios->Nombre->RowAttributes ?>>
		...
		<td<?php echo $usuarios->Nombre->CellAttributes() ?>><span class="ewSearchOpr">contiene<input type="hidden" name="z_Nombre" id="z_Nombre" value="LIKE"></span></td>
...
class cusuarios_search {
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $usuarios;
		$objForm = new cFormObj();
		if ($this->IsPageRequest()) { // Validate request

			// Get action
			$usuarios->CurrentAction = $objForm->GetValue("a_search");
			switch ($usuarios->CurrentAction) {
				case "S": // Get Search Criteria
					...
					if ($sSrchStr <> "") {
						$sSrchStr = $usuarios->UrlParm($sSrchStr);
						$this->Page_Terminate("usuarioslist.php" . "?" . $sSrchStr); // Go to list page
					}
			}
		}
		...
		$usuarios->RowType = EW_ROWTYPE_SEARCH;
		$this->RenderRow();
	}
...
function BuildAdvancedSearch() {
	global $usuarios;
	$sSrchUrl = "";
	$this->BuildSearchUrl($sSrchUrl, $usuarios->user_id); // user_id
	$this->BuildSearchUrl($sSrchUrl, $usuarios->usuario); // usuario
	$this->BuildSearchUrl($sSrchUrl, $usuarios->clave); // clave
	$this->BuildSearchUrl($sSrchUrl, $usuarios->Nombre); // Nombre
	return $sSrchUrl;
}
...
function BuildSearchUrl(&$Url, &$Fld) {
	...
	function LoadSearchValues() {
		...
		// Load search values
		$usuarios->user_id->AdvancedSearch->SearchValue = $objForm->GetValue("x_user_id");
		$usuarios->usuario->AdvancedSearch->SearchValue = $objForm->GetValue("x_usuario");
		$usuarios->clave->AdvancedSearch->SearchValue = $objForm->GetValue("x_clave");
		$usuarios->Nombre->AdvancedSearch->SearchValue = $objForm->GetValue("x_Nombre");
	}
	...
	function RenderRow() {
		...
		if ($usuarios->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// user_id
			$usuarios->user_id->EditCustomAttributes = "";
			$usuarios->user_id->EditValue = ew_HtmlEncode($usuarios->user_id->AdvancedSearch->SearchValue);

			// usuario
			$usuarios->usuario->EditCustomAttributes = "";
			$usuarios->usuario->EditValue = ew_HtmlEncode($usuarios->usuario->AdvancedSearch->SearchValue);

			// clave
			$usuarios->clave->EditCustomAttributes = "";
			$usuarios->clave->EditValue = ew_HtmlEncode($usuarios->clave->AdvancedSearch->SearchValue);

			// Nombre
			$usuarios->Nombre->EditCustomAttributes = "";
			$usuarios->Nombre->EditValue = ew_HtmlEncode($usuarios->Nombre->AdvancedSearch->SearchValue);
		}
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckInteger($usuarios->user_id->AdvancedSearch->SearchValue)) {
			if ($gsSearchError <> "") $gsSearchError .= "<br>";
			$gsSearchError .= "Entero Incorrecto - User Id";
		}

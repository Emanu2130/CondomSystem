...
// Define page object
$empleados_edit = new cempleados_edit();
$Page =& $empleados_edit;
...
empleados_edit.ValidateForm = function(fobj) {
...
empleados_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
empleados_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
empleados_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
empleados_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="arenas">Editar Modulo: Empleados<br><br>
<a href="<?php echo $empleados->getReturnUrl() ?>">Volver</a></span></p>
<?php $empleados_edit->ShowMessage() ?>
<form name="fempleadosedit" id="fempleadosedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return empleados_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="empleados">
...
<?php if ($empleados->id_empleado->Visible) { // id_empleado ?>
	<tr<?php echo $empleados->id_empleado->RowAttributes ?>>
		...
		<td<?php echo $empleados->id_empleado->CellAttributes() ?>><span id="el_id_empleado">
<div<?php echo $empleados->id_empleado->ViewAttributes() ?>><?php echo $empleados->id_empleado->EditValue ?></div><input type="hidden" name="x_id_empleado" id="x_id_empleado" value="<?php echo ew_HtmlEncode($empleados->id_empleado->CurrentValue) ?>">
</span><?php echo $empleados->id_empleado->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cempleados_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$empleados->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$empleados->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else {
			$empleados->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($empleados->id_empleado->CurrentValue == "")
		...
		switch ($empleados->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$empleados->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$empleados->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($empleados->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id_empleado
			$empleados->id_empleado->EditCustomAttributes = "";
			$empleados->id_empleado->EditValue = $empleados->id_empleado->CurrentValue;
			$empleados->id_empleado->CssStyle = "";
			$empleados->id_empleado->CssClass = "";
			$empleados->id_empleado->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $empleados->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($empleados->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$empleados->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}

...
// Define page object
$nomina_add = new cnomina_add();
$Page =& $nomina_add;

// Page init processing
$nomina_add->Page_Init();

// Page main processing
$nomina_add->Page_Main();
?>
...
// Create page object
var nomina_add = new ew_Page("nomina_add");

// page properties
nomina_add.PageID = "add"; // page ID
var EW_PAGE_ID = nomina_add.PageID; // for backward compatibility

// extend page with ValidateForm function
nomina_add.ValidateForm = function(fobj) {
...
nomina_add.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
nomina_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
nomina_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
nomina_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="arenas">Agregar Modulo: Nomina<br><br>
<a href="<?php echo $nomina->getReturnUrl() ?>">Volver</a></span></p>
<?php $nomina_add->ShowMessage() ?>
<form name="fnominaadd" id="fnominaadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return nomina_add.ValidateForm(this);">
<p>
...
<select id="x_empleado" name="x_empleado" onchange="ew_UpdateOpt('x_monto_pago','x_empleado',nomina_add.ar_x_monto_pago);ew_UpdateOpt('x_deducible_afp','x_empleado',nomina_add.ar_x_deducible_afp);ew_UpdateOpt('x_deducible_sf','x_empleado',nomina_add.ar_x_deducible_sf);"<?php echo $nomina->empleado->EditAttributes() ?>>
<?php
...
<script type="text/javascript">
<!--
nomina_add.ar_x_monto_pago = [<?php echo $jswrk ?>];
...
<script type="text/javascript">
<!--
nomina_add.ar_x_deducible_afp = [<?php echo $jswrk ?>];
...
<script type="text/javascript">
<!--
nomina_add.ar_x_deducible_sf = [<?php echo $jswrk ?>];
...
<!--
ew_UpdateOpts([['x_monto_pago','x_empleado',nomina_add.ar_x_monto_pago],
['x_deducible_afp','x_empleado',nomina_add.ar_x_deducible_afp],
['x_deducible_sf','x_empleado',nomina_add.ar_x_deducible_sf]]);
...
class cnomina_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $nomina->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$nomina->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $nomina->CurrentAction = "C"; // Copy Record
		  } else {
		    $nomina->CurrentAction = "I"; // Display Blank Record
		...
		switch ($nomina->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$nomina->SendEmail = TRUE; // Send email on add success
		...
		$nomina->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($nomina->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $nomina->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($nomina->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$nomina->id_nomina->setDbValue($conn->Insert_ID());
			$rsnew['id_nomina'] =& $nomina->id_nomina->DbValue;

			// Call Row Inserted event
			$nomina->Row_Inserted($rsnew);
		}
		return $AddRow;
	}

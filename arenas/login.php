...
login.ValidateForm = function(fobj)
{
	...
	if (!ew_HasValue(fobj.username))
		return ew_OnError(this, fobj.username, "Ingrese su usuario");
	if (!ew_HasValue(fobj.password))
		return ew_OnError(this, fobj.password, "Ingrese su clave");
...
<p><span class="arenas">Pagina de acceso</span></p>
<?php $login->ShowMessage() ?>
<form action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return login.ValidateForm(this);">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<td><span class="arenas">Usuario</span></td>
		<td><span class="arenas"><input type="text" name="username" id="username" size="20" value="<?php echo $login->sUsername ?>"></span></td>
	</tr>
	<tr>
		<td><span class="arenas">Clave</span></td>
		<td><span class="arenas"><input type="password" name="password" id="password" size="20"></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td><span class="arenas">
		<label><input type="radio" name="rememberme" id="rememberme" value="a"<?php if ($login->sLoginType == "a") { ?> checked="checked"<?php } ?>>Autoconectarme hasta que haga clic en salir</label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value="u"<?php if ($login->sLoginType == "u") { ?>  checked="checked"<?php } ?>>Guardar mi usuario</label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value=""<?php if ($login->sLoginType == "") { ?> checked="checked"<?php } ?>>Siempre pedirme el usuario y clave</label>
		</span></td>
	</tr>
	<tr>
		<td colspan="2" align="center"><span class="arenas"><input type="submit" name="submit" id="submit" value="  Acceso  "></span></td>
	</tr>
</table>
</form>
<br>
...
class clogin {
	...
	function Page_Main() {
		...
		if ($bValidate) {
			...
			if ($bValidate) {
				...
				if (!$bValidPwd)
					$this->setMessage("Usuario o clave incorrecta"); // Invalid User ID/password
			} else {
				if ($this->getMessage() == "")
	...
	function ValidateForm($usr, $pwd) {
		...
		if (trim($usr) == "") {
			...
			$gsFormError .= "Ingrese su usuario";
		}
		if (trim($pwd) == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "Ingrese su clave";
		}

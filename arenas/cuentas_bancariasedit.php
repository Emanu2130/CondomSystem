...
// Define page object
$cuentas_bancarias_edit = new ccuentas_bancarias_edit();
$Page =& $cuentas_bancarias_edit;

// Page init processing
$cuentas_bancarias_edit->Page_Init();

// Page main processing
$cuentas_bancarias_edit->Page_Main();
?>
...
// Create page object
var cuentas_bancarias_edit = new ew_Page("cuentas_bancarias_edit");

// page properties
cuentas_bancarias_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = cuentas_bancarias_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
cuentas_bancarias_edit.ValidateForm = function(fobj) {
...
cuentas_bancarias_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
cuentas_bancarias_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
cuentas_bancarias_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cuentas_bancarias_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="arenas">Editar Modulo: Cuentas Bancarias<br><br>
<a href="<?php echo $cuentas_bancarias->getReturnUrl() ?>">Volver</a></span></p>
<?php $cuentas_bancarias_edit->ShowMessage() ?>
<form name="fcuentas_bancariasedit" id="fcuentas_bancariasedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return cuentas_bancarias_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="cuentas_bancarias">
...
<?php if ($cuentas_bancarias->id_banco->Visible) { // id_banco ?>
	<tr<?php echo $cuentas_bancarias->id_banco->RowAttributes ?>>
		...
		<td<?php echo $cuentas_bancarias->id_banco->CellAttributes() ?>><span id="el_id_banco">
<div<?php echo $cuentas_bancarias->id_banco->ViewAttributes() ?>><?php echo $cuentas_bancarias->id_banco->EditValue ?></div><input type="hidden" name="x_id_banco" id="x_id_banco" value="<?php echo ew_HtmlEncode($cuentas_bancarias->id_banco->CurrentValue) ?>">
</span><?php echo $cuentas_bancarias->id_banco->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class ccuentas_bancarias_edit {
	...
	// Page Object Name
	var $PageObjName = 'cuentas_bancarias_edit';
	...
	//  - open connection
	//
	function ccuentas_bancarias_edit() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$cuentas_bancarias->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$cuentas_bancarias->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else {
			$cuentas_bancarias->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($cuentas_bancarias->id_banco->CurrentValue == "")
			$this->Page_Terminate("cuentas_bancariaslist.php"); // Invalid key, return to list
		switch ($cuentas_bancarias->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$cuentas_bancarias->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$cuentas_bancarias->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($cuentas_bancarias->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id_banco
			$cuentas_bancarias->id_banco->EditCustomAttributes = "";
			$cuentas_bancarias->id_banco->EditValue = $cuentas_bancarias->id_banco->CurrentValue;
			$cuentas_bancarias->id_banco->CssStyle = "";
			$cuentas_bancarias->id_banco->CssClass = "";
			$cuentas_bancarias->id_banco->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $cuentas_bancarias->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($cuentas_bancarias->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$cuentas_bancarias->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}

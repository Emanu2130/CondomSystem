...
// Define page object
$comprobantes_tipos_edit = new ccomprobantes_tipos_edit();
$Page =& $comprobantes_tipos_edit;

// Page init processing
$comprobantes_tipos_edit->Page_Init();

// Page main processing
$comprobantes_tipos_edit->Page_Main();
?>
...
// Create page object
var comprobantes_tipos_edit = new ew_Page("comprobantes_tipos_edit");

// page properties
comprobantes_tipos_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = comprobantes_tipos_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
comprobantes_tipos_edit.ValidateForm = function(fobj) {
...
comprobantes_tipos_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
comprobantes_tipos_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
comprobantes_tipos_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
comprobantes_tipos_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="arenas">Editar Modulo: Comprobantes Tipos<br><br>
<a href="<?php echo $comprobantes_tipos->getReturnUrl() ?>">Volver</a></span></p>
<?php $comprobantes_tipos_edit->ShowMessage() ?>
<form name="fcomprobantes_tiposedit" id="fcomprobantes_tiposedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return comprobantes_tipos_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="comprobantes_tipos">
...
<?php if ($comprobantes_tipos->id_comprobante->Visible) { // id_comprobante ?>
	<tr<?php echo $comprobantes_tipos->id_comprobante->RowAttributes ?>>
		...
		<td<?php echo $comprobantes_tipos->id_comprobante->CellAttributes() ?>><span id="el_id_comprobante">
<div<?php echo $comprobantes_tipos->id_comprobante->ViewAttributes() ?>><?php echo $comprobantes_tipos->id_comprobante->EditValue ?></div><input type="hidden" name="x_id_comprobante" id="x_id_comprobante" value="<?php echo ew_HtmlEncode($comprobantes_tipos->id_comprobante->CurrentValue) ?>">
</span><?php echo $comprobantes_tipos->id_comprobante->CustomMsg ?></td>
	</tr>
<?php } ?>
...
// Page Class
//
class ccomprobantes_tipos_edit {
	...
	// Page Object Name
	var $PageObjName = 'comprobantes_tipos_edit';
	...
	//  - open connection
	//
	function ccomprobantes_tipos_edit() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$comprobantes_tipos->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$comprobantes_tipos->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else {
			$comprobantes_tipos->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($comprobantes_tipos->id_comprobante->CurrentValue == "")
			$this->Page_Terminate("comprobantes_tiposlist.php"); // Invalid key, return to list
		switch ($comprobantes_tipos->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$comprobantes_tipos->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$comprobantes_tipos->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($comprobantes_tipos->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id_comprobante
			$comprobantes_tipos->id_comprobante->EditCustomAttributes = "";
			$comprobantes_tipos->id_comprobante->EditValue = $comprobantes_tipos->id_comprobante->CurrentValue;
			$comprobantes_tipos->id_comprobante->CssStyle = "";
			$comprobantes_tipos->id_comprobante->CssClass = "";
			$comprobantes_tipos->id_comprobante->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $comprobantes_tipos->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($comprobantes_tipos->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$comprobantes_tipos->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
